...
2. **Shop-core route**: если в локальном проекте подтверждены `catalog`, `sale` и `currency`, активируй интернет-магазин: товары, SKU/торговые предложения, цены, остатки, склады, корзина, checkout, заказы, оплаты, доставки, скидки, marketing/analytics (`sender`, `mail`, `messageservice`, `subscribe`, `advertising`, `abtest`, `conversion`, `report`, `statistic`), automation (`bizproc`, `bizprocdesigner`, `workflow`, `lists`, `pull`) и 1С/CommerceML. Новый shop truth layer проверен на core с `catalog` 25.550.0, `sale` 26.0.0, `currency` 26.0.0, `bitrix.eshop` 25.0.0.
...
- Для вопросов “все ли модули shop-core покрыты” сначала открывай `shop-core-module-inventory.md`: standard shop components, marketing/analytics и automation уже покрыты, но не обещай глубокое покрытие `webservice.sale`/`webservice.statistic` и runtime-smoke, пока отдельный reference/проверка не готовы.
