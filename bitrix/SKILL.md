...
- Интернет-магазин, SKU, цены, остатки, стандартные компоненты, корзина, checkout, заказы — [references/shop-task-matrix.md](references/shop-task-matrix.md), [references/shop-standard-components.md](references/shop-standard-components.md), [references/catalog.md](references/catalog.md), [references/sale.md](references/sale.md), [references/currency.md](references/currency.md), [references/commerce-workflows.md](references/commerce-workflows.md)
...
- Для вопросов “все ли модули shop-core покрыты” сначала открывай `shop-core-module-inventory.md`: standard shop components уже покрыты, но не обещай глубокое покрытие `sender`, `report`, `statistic`, `conversion`, `abtest`, `advertising`, `bizproc`, `webservice`, пока их отдельные references не готовы.
