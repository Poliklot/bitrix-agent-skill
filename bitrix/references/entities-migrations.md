...
$iblockId = $ib->Add([
    ...
    // --- D7 ORM (ВАЖНО: нужен для D7-работы с этим ИБ) ---
    'API_CODE'       => 'products', // уникальный, только [a-zA-Z0-9_], нужен для ElementProductsTable
...
// 'X' = полные права
// Для большинства задач: 'R' (гости), 'W' (редакторы), 'X' (администраторы)

CIBlock::SetPermission($iblockId, [
    1  => 'X',  // группа 1 = Администраторы
    2  => 'R',  // группа 2 = Все (Everyone) — публичное чтение
    // другие группы — без доступа ('D' или просто не указывать)
]);
...
// $right: '' = нет доступа, 'D' = нет, 'R' = чтение, 'W' = запись, 'X' = полный
...
$right = $APPLICATION->GetGroupRight('iblock', false, $groupId); // для конкретной группы
// или для текущего пользователя:
$right = $APPLICATION->GetGroupRight('iblock'); // для залогиненного пользователя
```
...
- **`API_CODE` задаётся один раз** — после создания инфоблока менять `API_CODE` через `CIBlock::Update` не рекомендуется: могут существовать скомпилированные классы с старым именем в кеше.
