...
// Вывести (в шаблоне — через компонент или вручную)
// Компонент: bitrix:breadcrumb  (стандартный)
// Или напрямую:
...
$this->addExternalCss($this->GetTemplatePath('style.css'));
$this->addExternalJs($this->GetTemplatePath('script.js'));
```
...
// Путь к шаблону
$this->GetTemplatePath('img/icon.png')  // полный путь от корня сайта
...
// Редирект
LocalRedirect('/new-url/', true);  // true = 301
LocalRedirect('/new-url/');        // 302
...
- `$this->GetTemplatePath('img.png')` возвращает URL, а не путь на сервере. Для серверного пути: `$_SERVER['DOCUMENT_ROOT'] . $this->GetTemplatePath('img.png')`
