...
Этот файл хранит безопасный снимок проекта: public root, состав модулей и версии, шаблоны, компоненты, `local/*`, события, агенты, tooling, shop/1С endpoints, REST/webservice, кеш/SEO/routing, риски и открытые вопросы. Следующий агент сначала читает `AGENTS.md`, затем `BITRIX_PROJECT_CONTEXT.md`, а потом сверяет рискованные факты с текущим кодом.
