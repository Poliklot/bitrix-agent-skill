...
## Project intake и файл контекста

Для реального проекта навык должен работать не только как справочник, но и как память по уже изученному коду. После полного аудита проекта агент должен создать или обновить в корне клиентского репозитория файл:

```text
BITRIX_PROJECT_CONTEXT.md
```

Этот файл хранит безопасный snapshot проекта: public root, состав модулей и версии, шаблоны, компоненты, `local/*`, события, агенты, tooling, shop/1С endpoints, REST/webservice, кеш/SEO/routing, риски и открытые вопросы. Следующий агент сначала читает `AGENTS.md`, затем `BITRIX_PROJECT_CONTEXT.md`, а потом сверяет рискованные факты с текущим кодом.
...
| Project intake | быстрый аудит реального проекта, `BITRIX_PROJECT_CONTEXT.md`, версии модулей, `local/*`, компоненты, шаблоны, events/agents, tooling |
