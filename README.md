...
- Закрывает shop-core baseline: 49 модулей inventory, `catalog`, `sale`, `currency`, SKU/торговые предложения, цены, остатки, склады, корзину, checkout, заказы, оплаты, доставки, скидки и 1С/CommerceML exchange — только после проверки модулей в конкретном проекте.
...
Полная папка `bitrix/` содержит update/install/uninstall-скрипты и 69 отдельных reference-файлов, поэтому превышает лимит MCP Market в 50 файлов. `mcpmarket/bitrix/` содержит тот же reference-слой, сгруппированный в compact bundles.
...
| `shop-core-module-inventory.md` | Полный inventory 49 модулей shop-core: версии, components/admin counts, shop/1С relevance, coverage status и очередь глубокого аудита (`storeassist`, shop components, marketing/analytics, automation, webservice) |
