...
Полная папка `bitrix/` содержит update/install/uninstall-скрипты и 74 отдельных reference-файла, поэтому превышает лимит MCP Market в 50 файлов. `mcpmarket/bitrix/` содержит тот же reference-слой, сгруппированный в compact bundles.
...
| `shop-core-module-inventory.md` | Полный inventory 49 модулей shop-core: версии, components/admin counts, shop/1С relevance, coverage status и очередь глубокого аудита (standard shop components, marketing/analytics, automation и webservice/REST уже закрыты, дальше runtime smoke) |
